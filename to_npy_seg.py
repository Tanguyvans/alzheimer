...
def resize_to_mni_resolution(input_path, output_path):
    """Resize a registered image to standard MNI resolution (91x109x91)."""
    logger.info("Resizing to standard MNI resolution...")
    
    # Load the input image
    input_img = ants.image_read(input_path)
    
    # Standard MNI dimensions
    mni_dims = (91, 109, 91)
...
def segment_hippocampus(input_image_path, output_folder):
    ...
    mask_left_img = nib.Nifti1Image(mask_left, input_img.affine, input_img.header)
    mask_right_img = nib.Nifti1Image(mask_right, input_img.affine, input_img.header)
    ...
    nib.save(nib.Nifti1Image(left_hippo, input_img.affine, input_img.header), left_hippo_path)
    nib.save(nib.Nifti1Image(right_hippo, input_img.affine, input_img.header), right_hippo_path)
...
def extract_hippocampus_slices(left_hippo_path, right_hippo_path, input_path, output_dir):
    """Extract a 3D volume containing only the coronal slices with hippocampus."""
    ...
    image_extracted = input_img.get_fdata()[:, hippo_slices, :]
    
    # Create new NIfTI image
    new_affine = input_img.affine.copy()
    new_affine[1, 3] = new_affine[1, 3] + (hippo_slices[0] * new_affine[1, 1])
    ...
    slice_info = {
        ...
        'slice_indices': hippo_slices,
        'total_hippocampus_slices': len(hippo_slices),
...
def main():
    ...
    config = {
        ...
        'input_path': os.path.join(BASE_DIR, "normal_1.nii.gz")  # Chemin vers votre fichier
    }
    ...
    try:
        ...
        # Step 1: Registration only (no resizing)
        ...
        # Utiliser directement registered_path pour les étapes suivantes
        # Step 2: Hippocampus Segmentation
        ...
        if not os.path.exists(npy_path):
            ...
            image = LoadImage(dtype=np.float32, image_only=True)(registered_path)
            image_dict = {"image": image}
