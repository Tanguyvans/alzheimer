#!/usr/bin/env python3
"""
Cross-Validation Training for ResNet3D + MLP Early Fusion (MRI + Tabular)

Runs K-fold cross-validation with multiple seeds for robust evaluation.
Same CV methodology as multimodal_fusion/train_cv.py with differential LR
and backbone freezing for the MedicalNet pretrained ResNet50 backbone.
...
class ResNet3DMLPTrainer:
    """Cross-validation trainer for ResNet3D + MLP Early Fusion"""
    ...
    def build_model(self) -> EarlyFusionModel:
        model = build_early_fusion_model(self.config)
        return model.to(self.device)

    def build_optimizer(self, model: EarlyFusionModel):
        """Build optimizer with differential LR: backbone gets 10x lower LR"""
        cfg = self.config['training']

        backbone_params = list(model.backbone.parameters())
        backbone_ids = set(id(p) for p in backbone_params)
        head_params = [p for p in model.parameters() if id(p) not in backbone_ids]
        backbone_lr = cfg['learning_rate'] * 0.1
    ...
    def train(self, model, train_loader, val_loader, optimizer, scheduler, criterion):
        ...
        for epoch in range(num_epochs):
            ...
            if epoch % 10 == 0 or epoch == num_epochs - 1:
                frozen_str = " [backbone frozen]" if epoch < freeze_backbone_epochs else ""
                logger.info(
            ...
            if self.use_wandb:
                wandb.log({
                    ...
                    f'fold_{self.fold}/lr_head': optimizer.param_groups[1]['lr'],
                    f'fold_{self.fold}/lr_backbone': optimizer.param_groups[0]['lr'],
                })
...
def run_cross_validation(config: Dict, n_folds: int, seeds: List[int], output_dir: Path,
    ...
    if use_wandb and WANDB_AVAILABLE:
        ...
        wandb.init(
            project=wandb_config.get('project', 'alzheimer-resnet3d-mlp'),
            name=wandb_config.get('name', f'resnet3d_mlp_cv_{n_folds}fold'),
            config=config
        )
    ...
    for seed in seeds:
        ...
        for fold, (train_idx, test_idx) in enumerate(skf.split(all_indices, all_labels)):
            ...
            trainer = ResNet3DMLPTrainer(config, fold, seed, fold_dir, use_wandb=use_wandb)
            ...
            logger.info(f"EarlyFusion params: {total_params:,}")
            ...
            logger.info(
                f"  Val: {best_val_acc:.1f}% | "
                f"Traj: Acc={test_metrics['accuracy']:.1f}%, "
    ...
    # Final summary
    logger.info(f"\n{'='*60}")
    logger.info("FINAL CROSS-VALIDATION RESULTS (ResNet3D + MLP)")
    logger.info(f"{'='*60}")
    ...
    summary = {
        'experiment': 'resnet3d_mlp_early_fusion',
        'n_folds': n_folds,
        'seeds': seeds,
...
def main():
    parser = argparse.ArgumentParser(description='Cross-Validation for ResNet3D + MLP Early Fusion')
