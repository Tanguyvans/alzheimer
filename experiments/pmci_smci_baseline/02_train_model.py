"""
Train DenseNet3D for pMCI vs sMCI Binary Classification
...
class ResNet3DMCIBinary(pl.LightningModule):
    ...
    def __init__(
        ...
        self.classifier = CustomClassifier(
            ...
            num_class=2,  # Binary: sMCI (0) vs pMCI (1)
            task="multiclass",
    ...
    def validation_step(self, batch, batch_idx):
        ...
        probs = torch.softmax(logits, dim=1)[:, 1]  # Probability of pMCI (class 1)
    ...
    def on_validation_epoch_end(self):
        ...
        logger.info(f"  Sensitivity (pMCI recall): {sensitivity if 'sensitivity' in locals() else 0:.4f}")
        logger.info(f"  Specificity (sMCI recall): {specificity if 'specificity' in locals() else 0:.4f}")
    ...
    def on_test_epoch_end(self):
        ...
        logger.info(f"  TN={cm[0,0]}, FP={cm[0,1]}")
        logger.info(f"  FN={cm[1,0]}, TP={cm[1,1]}")
        logger.info(f"\n  Sensitivity (pMCI recall): {sensitivity:.4f}")
        logger.info(f"  Specificity (sMCI recall): {specificity:.4f}")
        logger.info(f"{'='*80}\n")
