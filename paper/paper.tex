...
Experiments used 5-fold cross-validation to assess stability.
...
To quantify the contribution of each modality, we evaluated single-modality baselines using identical training protocols on the AD-trajectory task (Table~\ref{tab:ablation}). The MRI-only model (ViT with MAE pretraining) achieved 84.1\% accuracy. The tabular-only model (FT-Transformer) achieved 83.5\% accuracy.

The multimodal fusion achieves substantial gains over both unimodal baselines: \textbf{+8.0\%} accuracy over MRI-only and \textbf{+8.6\%} over tabular-only. This confirms that the modalities provide complementary information that cross-attention fusion effectively captures.
...
\begin{tabular}{lcc}
\toprule
\textbf{Model} & \textbf{Acc.} & \textbf{$\Delta$ Acc.} \\
\midrule
MRI only (ViT) & 84.1\% & --- \\
Tabular only (FT-T) & 83.5\% & --- \\
\midrule
\textbf{Multimodal (Ours)} & \textbf{92.1\%} & \textbf{+8.0\%} \\
\bottomrule
\end{tabular}
\label{tab:ablation}
\end{center}
\vspace{-2mm}
{\scriptsize Note: Results on AD-trajectory test set. Cross-task evaluation on stable AD achieves 94.1\% (Table~\ref{tab:results}).}
\end{table}
