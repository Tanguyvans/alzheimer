...
The same trajectory-trained model achieved \textbf{93.3\% accuracy} (AUC: 0.956) on the stable AD test set. While cross-task performance is slightly lower than in-domain, this demonstrates that training on heterogeneous presentations---including subtle early-stage MCI converters---produces robust features that generalize effectively to established disease detection.
...
\midrule
AD-trajectory (in-domain) & 92.4\% & 78.8\% & 96.2\% & 0.959 \\
\textbf{Stable AD (cross-task)} & \textbf{93.3\%} & \textbf{79.0\%} & \textbf{96.2\%} & \textbf{0.956} \\
\bottomrule
\end{tabular}
\label{tab:results}
\end{center}
...
Tabular only (FT-T) & 84.9$\pm$0.4\% & 84.4$\pm$0.3\% & 0.925 \\
\midrule
\textbf{Multimodal (Ours)} & \textbf{92.4$\pm$1.0\%} & \textbf{87.5$\pm$2.5\%} & \textbf{0.959} \\
\bottomrule
\end{tabular}
\label{tab:ablation}
\end{center}
...
Figure~\ref{fig:roc} shows the ROC curves comparing multimodal fusion against unimodal baselines. The multimodal model achieves substantially higher AUC (0.959) than both MRI-only and tabular-only models, demonstrating the benefit of cross-modal attention fusion.

\begin{figure}[htbp]
\centerline{\includegraphics[width=\columnwidth]{figures/roc_curves.pdf}}
\caption{ROC curves comparing multimodal fusion (AUC=0.959) with tabular-only (AUC=0.925) and MRI-only baselines on the AD-trajectory test set. The multimodal model achieves superior discrimination across all operating points.}
\label{fig:roc}
\end{figure}
