...
\textbf{ADNI (Alzheimer's Disease Neuroimaging Initiative):} Data used in the preparation of this article were obtained from the ADNI database (adni.loni.usc.edu). ADNI is a longitudinal study collecting multimodal neuroimaging and clinical data to track Alzheimer's disease progression. We included 903 subjects (424 CN, 479 AD-trajectory).
...
Missing values were imputed using median imputation and all features were z-score normalized. Importantly, cognitive screening scores (MMSE, CDR) were excluded, as clinicians use these scores to diagnose AD, making them unsuitable as predictive features.
...
\textbf{Cross-Modal Fusion:} Both modality representations are fused using bidirectional cross-attention with 8 heads and hidden dimension 512. Each cross-attention layer includes residual connections and feed-forward networks:
\begin{align}
...
\end{align}
This bidirectional attention enables each modality to attend to complementary information from the other. Auxiliary classifiers on each branch (weighted 0.3) encourage modality-specific learning.
...
\begin{itemize}
    \item Weighted cross-entropy loss with label smoothing ($\epsilon$ = 0.1)
    \item Auxiliary losses on modality-specific branches (weight = 0.3)
    \item Test-time augmentation with 8 geometric transformations
\end{itemize}

\textbf{Implementation:}
\begin{itemize}
    \item Batch size: 4 (limited by 3D volume memory requirements)
\end{itemize}
...
\begin{itemize}
    ...
    \item \textbf{Balanced Accuracy:} Average of sensitivity and specificity, accounting for class imbalance
    \item \textbf{Sensitivity:} True positive rate (recall for AD-trajectory class)
    \item \textbf{Specificity:} True negative rate (recall for CN class)
    \item \textbf{AUC:} Area under the receiver operating characteristic curve
\end{itemize}

Results are reported as mean $\pm$ standard deviation across 15 runs (5 folds $\times$ 3 seeds).
