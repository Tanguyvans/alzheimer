...
Combining imaging with clinical data improves classification. Qiu et al. \cite{qiu2020} achieved AUC 0.996 on ADNI using an end-to-end FCN-MLP framework. In a larger follow-up study, Qiu et al. \cite{qiu2022} achieved 95\% accuracy on 8,916 subjects using CNN features with CatBoost for tabular data as separate models. No prior work has combined 3D ViT with FT-Transformer in an end-to-end multimodal framework for AD classification.
...
Our multimodal model achieved 93.3\% accuracy and 87.9\% balanced accuracy on the AD-trajectory test set (Table~\ref{tab:results}). The gap between accuracy and balanced accuracy reflects the class imbalance (78\% CN vs 22\% AD-trajectory).

Subgroup analysis (Table~\ref{tab:subgroups}) reveals important patterns: CN cases are classified with very high accuracy (97.5\%), while the heterogeneous AD-trajectory group is more challenging. Within AD-trajectory, established AD cases achieve 97.1\% accuracy, while MCI-to-AD converters are harder (82.1\%). This is expected---MCI converters represent early-stage disease with subtler structural changes.

\begin{table}[htbp]
\caption{Test Set Performance on AD-Trajectory Task}
\begin{center}
\begin{tabular}{lc}
\toprule
...
Sensitivity (AD-traj recall) & 78.3\% \\
Specificity (CN recall) & 97.5\% \\
\bottomrule
\end{tabular}
\label{tab:results}
\end{center}
...
\begin{tabular}{lcc}
\toprule
\textbf{Subgroup} & \textbf{Accuracy} & \textbf{N} \\
\midrule
Cognitively Normal (CN) & 97.5\% & 712 \\
Alzheimer's Disease (AD) & 97.1\% & 34 \\
MCI $\rightarrow$ AD converters & 82.1\% & 39 \\
AD-trajectory (other) & 72.0\% & 125 \\
\bottomrule
\end{tabular}
\label{tab:subgroups}
\end{center}
...
A key question for clinical deployment is whether models trained to detect the full AD spectrum (including MCI converters) can accurately identify established AD cases. We performed a cross-task evaluation: training on cn\_ad\_trajectory (CN vs AD-trajectory) and testing on a held-out set containing only clinically confirmed, stable AD patients.

Remarkably, the trajectory-trained model achieved \textbf{96.1\% accuracy} and \textbf{92.1\% balanced accuracy} on stable AD detection (Table~\ref{tab:crosstask}), substantially exceeding its performance on the mixed trajectory test set (93.3\%). This suggests that training on heterogeneous AD presentations---including subtle early-stage MCI converters---produces more robust features for detecting established disease.

\begin{table}[htbp]
\caption{Cross-Task Evaluation: Train on AD-Trajectory, Test on Stable AD}
\begin{center}
\begin{tabular}{lcccc}
\toprule
...
The improved cross-task performance can be attributed to the training distribution: MCI converters exhibit subtle, early-stage atrophy patterns that force the model to learn fine-grained discriminative features. When evaluated on established AD cases with more pronounced atrophy, these features generalize effectively.
...
\textbf{Ours (stable AD)} & MRI+Tab & 3 & 6,065 & \textbf{96.1\%} \\
\textbf{Ours (AD-traj)} & MRI+Tab & 3 & 6,065 & 93.3\% \\
Qiu et al. \cite{qiu2022} & MRI+Tab & 8 & 8,916 & 95\% \\
...
Current gold-standard AD diagnosis relies on invasive biomarkers: cerebrospinal fluid (CSF) analysis requires lumbar puncture, while amyloid PET imaging involves radioactive tracers and costs \$3,000--5,000 per scan \cite{alzfacts2024}. These procedures, while highly accurate (CSF biomarker panels achieve 85--95\% sensitivity/specificity \cite{blennow2010}), are impractical for routine screening.

Our approach uses only non-invasive measures---structural MRI and cognitive assessments---achieving 96.1\% accuracy on stable AD detection. This matches or exceeds invasive biomarker accuracy without the associated risks, discomfort, and costs. Such a model could serve as a screening tool in clinical practice: patients with high predicted AD probability would be referred for confirmatory biomarker testing, while low-risk patients could avoid unnecessary invasive procedures.

The 80-85\% benchmark established by systematic reviews may be conservative; with modern transformer architectures, multimodal fusion, and rigorous methodology, non-invasive approaches can achieve clinically actionable accuracy competitive with invasive biomarkers.
...
\bibitem{alzfacts2024} Alzheimer's Association, ``2024 Alzheimer's disease facts and figures,'' \textit{Alzheimer's Dement.}, vol. 20, no. 5, pp. 3708--3821, 2024.

\bibitem{blennow2010} K. Blennow, H. Hampel, M. Weiner, and H. Zetterberg, ``Cerebrospinal fluid and plasma biomarkers in Alzheimer disease,'' \textit{Nat. Rev. Neurol.}, vol. 6, no. 3, pp. 131--144, 2010.
