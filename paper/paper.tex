...
\begin{abstract}
Deep learning for Alzheimer's disease (AD) classification from MRI has shown promise, yet recent reviews reveal that fewer than 20\% of studies employ external validation, with data leakage (e.g., same subjects in train and test sets) inflating reported accuracies to 95-99\%. Studies with rigorous subject-level splitting achieve 66-90\%. We present an end-to-end multimodal framework combining Vision Transformer (ViT) for 3D MRI analysis and Feature Tokenizer Transformer (FT-Transformer) for clinical tabular data, integrated through bidirectional cross-attention. We validate on 6,065 subjects from three independent cohorts (ADNI, OASIS, NACC) using strict subject-level 5-fold cross-validation. Our approach achieves \textbf{92.4\% accuracy} (AUC: 0.96) for distinguishing cognitively normal subjects from AD-trajectory patients (including MCI patients who later developed AD), and \textbf{93.3\%} on established AD cases. External validation reveals that single-cohort models suffer severe performance degradation (accuracy drops of 25-43 points), while our multi-cohort approach maintains robust generalization.
\end{abstract}
...
\textbf{ADNI (Alzheimer's Disease Neuroimaging Initiative):} A longitudinal study collecting multimodal neuroimaging and clinical data to track Alzheimer's disease progression. We included 903 subjects (424 CN, 479 AD-trajectory).
...
\textbf{NACC (National Alzheimer's Coordinating Center):} The largest US Alzheimer's database, aggregating data from over 40 Alzheimer's Disease Research Centers (ADRCs). MRI data were obtained from SCAN (Standardized Centralized Alzheimer's \& Related Dementias Neuroimaging), NACC's centralized neuroimaging repository. We included 4,132 subjects (3,581 CN, 551 AD-trajectory).
...
Our classification groups established AD patients with MCI subjects who subsequently progressed to AD (MCI converters). This grouping is grounded in the 2018 NIA-AA Research Framework \cite{jack2018}, which defines AD as a biological continuum rather than a clinical syndrome---individuals with AD pathology are on the ``Alzheimer's continuum'' regardless of clinical stage. Longitudinal neuroimaging studies have demonstrated that MCI converters exhibit AD-like atrophy patterns detectable on structural MRI at baseline \cite{jack2000,misra2009}.
...
\midrule
\multicolumn{8}{@{}l}{\textit{Proper subject-level splitting:}} \\
\textbf{Ours (estab)} & MRI+Tab & 3 & 6,065 & \textbf{93.3} & \textbf{79.0} & \textbf{96.2} & \textbf{.96} \\
\textbf{Ours (traj)} & MRI+Tab & 3 & 6,065 & 92.4 & 78.8 & 96.2 & .96 \\
Qiu \cite{qiu2022} & MRI+Tab & 8 & 8,916 & ---$^*$ & --- & --- & .94--.97$^*$ \\
VECNN \cite{vecnn2024} & MRI & 1 & 818 & 94.1 & 95.7 & 97.1 & --- \\
Wen \cite{wen2020} & MRI & 3 & 1k & 89$^\dag$ & --- & --- & --- \\
\midrule
\multicolumn{8}{@{}l}{\textit{Potential data leakage:}} \\
